...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-6 md:px-10 h-20 bg-white/80 backdrop-blur-md border-b border-black/5">
      ...
      <nav className="hidden md:flex items-center bg-[#F2F2F0] rounded-full px-1.5 py-1.5 gap-0.5">
        {navLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className={`px-3 lg:px-5 py-2 rounded-full text-xs lg:text-sm font-medium transition-all duration-200 whitespace-nowrap ${
              isActive(link.href)
      ...
      <button
        ...
        className="hidden md:flex items-center gap-2 bg-[#1A1A1A] hover:bg-[#333] text-white text-xs lg:text-sm font-semibold px-4 lg:px-6 py-3 rounded-full transition-all duration-200 shrink-0"
      >
        Book a 15min Call
      </button>
